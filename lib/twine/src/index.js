...
/* Try to open story preloaded in editme div */

    try {
      const importHTML = require('./data/import');
      const { deleteStory, importStory } = require('./data/actions/story');
      import_text = document.querySelector('#editme tw-storydata').outerHTML;
      const storyData = importHTML(import_text);
      if (storyData.length > 0)
      {
        storyData.forEach(new_story => {
          old_stories = store.state.story.stories.filter(old_story => old_story.name === new_story.name)
          old_stories.forEach(old_story => { 
                  /* store.state.story.stories.$remove(old_story); */
                  deleteStory(store, old_story.id); });
          importStory(store, new_story); 
          id = new_story.id
        });
        if (id) {
          window.location.hash = '#stories/' + id;
          // window.location.reload();
          document.querySelector('#editme').innerHTML = '';
        }
      }
    } catch (e) {
      err = e.message;
      debugger;
      /* did not find a story to import or had a problem importing it
      */
    }
